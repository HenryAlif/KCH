...
class Data(Base):
    ...
    def kirim_data(cls, session_postgresql, h_value, d_value, t_value, status, code_instrument, time_series):
        ...
        # Kirim data ke MySQL
        kirim_data_mysql(h_value, d_value, t_value, status, code_instrument, time_series)

def kirim_data_mysql(h_value, d_value, t_value, status, code_instrument, time_series):
    if conn_mysql:
        try:
            query_mysql = """
...
class Log(Base):
    ...
    def kirim_log(cls, session_postgresql, error, time):
        ...
        # Kirim log ke MySQL
        kirim_log_mysql(error, time)

def kirim_log_mysql(error, time):
    if conn_mysql:
        try:
            query_mysql = """
