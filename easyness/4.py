...
db_url_postgresql = "postgresql://users_pims_engineer:Engineer_2023@10.106.1.40/pims_prod"
...
class Data(Base):
    ...
    @classmethod
    def kirim_data_postgresql(cls, session_postgresql, h_value, d_value, t_value, status, code_instrument, time_series):
    ...
    @classmethod
    def kirim_data_mysql(cls, h_value, d_value, t_value, status, code_instrument, time_series):
        if conn_mysql:
            try:
                query_mysql = """
    ...
    def kirim_data(cls, session_postgresql, h_value, d_value, t_value, status, code_instrument, time_series):
        # Kirim data ke PostgreSQL
        cls.kirim_data_postgresql(session_postgresql, h_value, d_value, t_value, status, code_instrument, time_series)

        # Kirim data ke MySQL dengan mekanisme retry
        max_retries = 3
        for attempt in range(max_retries):
            try:
                cls.kirim_data_mysql(h_value, d_value, t_value, status, code_instrument, time_series)
                break
            except Exception as ex:
                logging.error(f"Retry {attempt + 1} failed: {ex}")
                time.sleep(2)
        else:
            logging.error("Failed to insert data into MySQL after multiple retries.")
    ...
    def kirim_log(cls, session_postgresql, error, time):
        ...
        if conn_mysql:
            try:
                ...
                conn_mysql.commit()
                logging.info("Log inserted into MySQL.")
            except mysql.connector.Error as err:
                logging.error(f"Error inserting log into MySQL: {err}")
