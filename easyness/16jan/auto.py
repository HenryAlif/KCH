...
if __name__ == "__main__":
    ...
    server_thread = threading.Thread(
        target=run_program, 
        args=("Server", ["python", "D:\\Source Code\\Magang Kalbe\\Raspberry Pi\\easyness\\16jan\\servertrigger.py"])
    )
    ...
    query_thread = threading.Thread(
        target=run_program, 
        args=("Query", ["python", "D:\\Source Code\\Magang Kalbe\\Raspberry Pi\\easyness\\16jan\\querydatabase.py"])
    )
    server_thread.start()
    client_thread.start()
    query_thread.start()
